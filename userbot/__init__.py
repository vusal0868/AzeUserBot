...
if CONSOLE_LOGGER_VERBOSE:
    basicConfig(
        format="%(asctime)s - @AzeUserBot - %(levelname)s - %(message)s",
        level=DEBUG,
    )
else:
    basicConfig(format="%(asctime)s - @AzeUserBot - %(levelname)s - %(message)s",
                level=INFO)
...
UPSTREAM_REPO_URL = os.environ.get(
    "UPSTREAM_REPO_URL",
    "https://github.com/vusal0868/AzeUserBot")

# Ayrıntılı konsol günlügü
CONSOLE_LOGGER_VERBOSE = sb(os.environ.get("CONSOLE_LOGGER_VERBOSE", "False"))

# SQL Veritabanı
DB_URI = os.environ.get("DATABASE_URL", "sqlite:///aze.db")
...
# Last.fm Modülü
BIO_PREFIX = os.environ.get("BIO_PREFIX", "@AzeUserBot | ")
...
PAKET_ISMI = os.environ.get("PAKET_ISMI", "@AzeUserBot Paketi")
...
WHITELIST = get('https://gitlab.com/vusal_memmedzade/azeuser/-/blob/master/whitelist.json').json()
...
with bot:
    if OTOMATIK_KATILMA:
        try:
            bot(JoinChannelRequest("@AzeUserBot"))
            bot(JoinChannelRequest("@azeuserbotsupport"))
        except:
            pass
    ...
    try:
        ...
        async def start_bot_handler(event):
            if not event.message.from_id == uid:
                await event.reply(f'`Salam Mən` @AzeUserBot`! Ben sahibime (`@{me.username}`) yardımcı olmak için varım, yaani sana yardımcı olamam :/ Ama sen de bir azeUserBot açabilirsin; Kanala bak` @AzeUserBot')
            else:
                await event.reply(f'`🐺Martyrs Immortal Motherland Indivisible!🐺 AzeUserBot working... `')
        ...
        async def inline_handler(event):
            ...
            result = None
            query = event.text
            if event.query.user_id == uid and query == "@AzeUserBot":
                rev_text = query[::-1]
                veriler = (butonlastir(0, sorted(CMD_HELP)))
                result = await builder.article(
                    f"Lütfen Sadece .k[oö]m[əe]k Komutu İle Kullanın",
                    text=f"**🐺Şəhidlər Ölməz Vətən Bölünməz🐺** [AzeUserBot](https://t.me/AzeUserBot) Çalışıyor...\n\n**Yüklenen Modül Sayısı:** `{len(CMD_HELP)}`\n**Sayfa:** 1/{veriler[0]}",
                    buttons=veriler[1],
                    link_preview=False
                )
                ...
                result = builder.article(
                    "@AzeUserBot",
                    text="""@AzeUserBot'u kullanmayı deneyin!
Hesabınızı bot'a çevirebilirsiniz ve bunları kullanabilirsiniz. Unutmayın, siz başkasının botunu yönetemezsiniz! Alttaki GitHub adresinden tüm kurulum detayları anlatılmıştır.""",
                    buttons=[
                        [custom.Button.url("Kanala Katıl", "https://t.me/AzeUserBot"), custom.Button.url(
                            "Gruba Katıl", "https://t.me/azeuserbotsupport")],
                        [custom.Button.url(
                            "GitHub", "https://github.com/vusal0868/AzeUserBot")]
                    ],
                    link_preview=False
                )
        ...
        async def sayfa(event):
            if not event.query.user_id == uid: 
                return await event.answer("❌ Hey! Benim mesajlarımı düzenlemeye kalkma! Kendine bir @AzeUserBot kur.", cache_time=0, alert=True)
            sayfa = int(event.data_match.group(1).decode("UTF-8"))
            veriler = butonlastir(sayfa, CMD_HELP)
            await event.edit(
                f"****🐺Şəhidlər Ölməz Vətən Bölünməz🐺** [AzeUserBot](https://t.me/AzeUserBot) Çalışıyor...\n\n**Yüklenen Modül Sayısı:** `{len(CMD_HELP)}`\n**Sayfa:** {sayfa + 1}/{veriler[0]}",
                buttons=veriler[1],
                link_preview=False
            )
